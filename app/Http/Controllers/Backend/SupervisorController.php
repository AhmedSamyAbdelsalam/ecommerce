...
class SupervisorController extends Controller
{
    ...
    {
        if( !auth()->user()->ability('admin','show_supervisors') ) {
    ...
    public function show(SupervisorRequest $request)
    {
    ...
    public function edit(User $supervisor)
    {
        ...
        return view('backend.supervisors.edit',compact('supervisor','permissions','userPermissions'));
    }
    ...
    public function update(SupervisorRequest $request, User $supervisor)
    {
        ...
        if($image = $request->file('user_image'))
        {
            if($supervisor->user_image != '' && File::exists('assets/users'.$supervisor->user_image)){
                unlink( File::exists('assets/users'.$supervisor->user_image));
            }
            $filename = Str::slug($request->username).'.' .$image->getClientOriginalExtension();
            $path = public_path('assets/users/'. $filename);
            Image::make($image->getRealPath())->resize(300, null, function ($constraint) {
