...
class CustomerController extends Controller
{
    ...
    {
        if( !auth()->user()->ability('admin','show_customers') ) {
    ...
    public function update(CustomerRequest $request, User $customer)
    {
        ...
        if($image = $request->file('user_image'))
        {
            if($customer->user_image != '' && File::exists('assets/users'.$customer->user_image)){
                unlink( File::exists('assets/users'.$customer->user_image));
            }
            $filename = Str::slug($request->username).'.' .$image->getClientOriginalExtension();
            $path = public_path('assets/users/'. $filename);
            Image::make($image->getRealPath())->resize(300, null, function ($constraint) {
