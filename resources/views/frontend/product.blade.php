   ...
   <section class="py-5">
        <div class="container">
          <div class="row mb-5">
                    ...
                    <div class="swiper-wrapper">
                        @foreach($product->media as $media)
                            <div class="swiper-slide h-auto">
                                <a class="glightbox product-view" href="{{asset('assets/products/'.$media->file_name)}}" data-gallery="gallery2" data-glightbox="Product item 1">
            ...
            <div class="col-lg-6">
              ...
              <p class="text-sm mb-4">{{$product->description}}</p>
                <livewire:frontend.show-prouduct :product="$product"/>

                <br>
                <ul class="list-unstyled small d-inline-block">
                <li class="px-3 py-2 mb-1 bg-white text-muted"><strong class="text-uppercase text-dark">Category:</strong>
                    <a class="reset-anchor ms-2" href="{{route('frontend.shop',$product->category->slug)}}">{{$product->category->name}}</a></li>
                      <li class="px-3 py-2 mb-1 bg-white text-muted">
                          <strong class="text-uppercase text-dark">Tags:</strong>
                          @foreach($product->tags as $tag)
                              <a class="reset-anchor ms-2" href="{{route('frontend.shop_tag',$tag->slug)}}">{{$tag->name}}</a>
                          @endforeach
                      </li>
                </ul>
            </div>
          ...
          <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
            <li class="nav-item">
                <a class="nav-link text-uppercase active" id="description-tab" data-toggle="tab" href="#description" role="tab" aria-controls="description" aria-selected="true">
                    Description
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-uppercase" id="reviews-tab" data-toggle="tab" href="#reviews" role="tab" aria-controls="reviews" aria-selected="false">
                    Reviews
                </a>
            </li>
          </ul>
